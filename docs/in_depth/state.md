...
- `mutate_with`
  ...
  功能：允许用户传入一个用于修改状态变量的函数，在修改完状态变量后，返回状态变量的引用。
...
序列容器支持迭代。在迭代时，需要先调用序列容器的`iter`接口生成迭代器，并配合`for`等语法完成对序列容器的迭代，如下列代码所示：
...
.. admonition:: 注意
   ...
   序列容器的长度并不能无限增长，其上限为2\sup:`32` - 1（4294967295，约为42亿）。
```
...
.. admonition:: 注意
   ...
   映射容器中键值对的个数并不能无限增长，其上限为2\sup:`32` - 1（4294967295，约为42亿）。
```
...
可迭代映射容器支持迭代。在迭代时，需要先调用可迭代映射容器的`iter`接口生成迭代器，迭代器在迭代时会不断返回由键的引用及对应的值的引用组成的元组，可配合`for`等语法完成对可迭代映射容器的迭代，如下列代码所示：
